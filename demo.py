...
import sys
import os
from word2vec import Word2Vec, Sent2Vec, LineSentence, Doc2Vec
...
#	sent_file = 'music_review_text.txt'
#	model = Sent2Vec(LineSentence(sent_file), model_file=input_file + '.model')
#	model.save_sent2vec_format(sent_file + '.vec')

#	doc_file = 'music_review_text.txt'
#	model = Doc2Vec(LineSentence(input_file), size=100, window=5, sg=0, min_count=5, workers=8)
#	model.save(input_file + '.model')
